...
case 'x':
        ...
        if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
        pemuda.toggleDisappearingMessages(from, 0)
        sendButLocation(from, `${ngazap(prefix)}`, `${virtag(prefix)}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}bbaij72njnwjibdo16830nslm1782`,buttonText:{disText:'⚔️ Pemuda Teknologi ⚔️'},type:1}])
        pemuda.toggleDisappearingMessages(from, 0)
break
case 'bugrow':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
